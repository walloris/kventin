...
`uuid,tubNum`. Новые пары дописываются в этот файл только один раз. Рядом
создаётся `uuid_tubnum.errors.csv`, который при каждом запуске атомарно
перезаписывается и содержит не более одной актуальной ошибки на UUID.

Error CSV содержит колонки `uuid,source_row,category,retryable,error`.
`retryable=false` означает конечный исход (`not_found`, `no_tab_num`,
`record_unavailable` или `invalid_input`): при следующем запуске такой UUID
пропускается без запроса. `retryable=true` означает временную сетевую/API
ошибку: она будет повторена. Если повтор завершился успешно, старая ошибка
удаляется. Старый трёхколоночный error CSV автоматически классифицируется,
схлопывается по UUID и переводится в новый формат при первом запуске.
