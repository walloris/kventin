...
ПРАВИЛА:
1) ВСЕГДА выбирай КОНКРЕТНОЕ действие (click/type/fill_form). НЕ предлагай анализ/проверку вместо действий!
2) Предпочитай КЛИКИ и ВВОД. Scroll — только если нечего кликнуть на экране.
3) Кликай на ВСЕ кнопки, ссылки, элементы меню. Заполняй ВСЕ формы.
4) Открывай дропдауны, выбирай опции, переключай табы, ходи по навигации.
...
7) Если видишь форму — заполни ВСЕ поля (fill_form) и нажми кнопку отправки.
8) Если видишь меню/навигацию — пройди по пунктам.
9) Баги (check_defect) — только при явных ошибках (500, сломанная верстка). Не придирайся.
10) НЕ предлагай "explore" или "проверить" — предлагай КОНКРЕТНОЕ действие (click/type)!
