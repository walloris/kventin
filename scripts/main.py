...
Агент непрерывно анализирует страницу, использует local OpenAI-compatible LLM
как необязательный источник решений и продолжает работу по локальной policy.
...
def main():
    ...
    #   python scripts/daemon.py            # цикл раз в 15 минут
    #   python scripts/daemon.py --once     # один прогон (cron)
